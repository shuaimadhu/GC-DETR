...
__all__ = ['HybridEncodermobilemamba']
...
########################################################################################################################
from .mobilemamba import MobileMambaBlock

class MobileCSPLayer(nn.Module):
    def __init__(self, in_channels, out_channels, num_blocks=1, expansion=1, bias=False, act='silu', bottletype='s', global_r=0.8, local_r=0.2):
        super(MobileCSPLayer, self).__init__()
        hidden_channels = int(out_channels * expansion)
        self.conv1 = ConvNormLayer_fuse(in_channels, hidden_channels, 1, 1, bias=bias, act=act)
        self.conv2 = ConvNormLayer_fuse(in_channels, hidden_channels, 1, 1, bias=bias, act=act)
        self.bottlenecks = nn.Sequential(*[
            MobileMambaBlock(type=bottletype, ed=hidden_channels, global_ratio=global_r, local_ratio=local_r, kernels=5, ssm_ratio=1) 
...
class MobilemambaRepNCSPELAN4(nn.Module):
    # csp-elan
    def __init__(self, c1, c2, c3, c4, n=3,
                 bias=False,
                 act="silu", gr=None, lr=None):
        super().__init__()
        self.c = c3//2
        self.cv1 = ConvNormLayer_fuse(c1, c3, 1, 1, bias=bias, act=act)
        self.cv2 = nn.Sequential(MobileCSPLayer(c3//2, c4, n, 1, bias=bias, act=act, bottletype='s', global_r=gr, local_r=lr), ConvNormLayer_fuse(c4, c4, 3, 1, bias=bias, act=act))
        self.cv3 = nn.Sequential(MobileCSPLayer(c4, c4, n, 1, bias=bias, act=act, bottletype='s', global_r=gr, local_r=lr), ConvNormLayer_fuse(c4, c4, 3, 1, bias=bias, act=act))
        self.cv4 = ConvNormLayer_fuse(c3+(2*c4), c2, 1, 1, bias=bias, act=act)
...
########################################################################################################################
# from .rmt import RetBlock, RelPos2d
# class retCSPLayer(nn.Module):
#     def __init__(self, in_channels, out_channels, num_blocks=1, expansion=1, bias=False, act='silu', retention='chunk', num_heads=8):
#         super(retCSPLayer, self).__init__()
#         hidden_channels = int(out_channels * expansion)
#         self.conv1 = ConvNormLayer_fuse(in_channels, hidden_channels, 1, 1, bias=bias, act=act)
#         self.conv2 = ConvNormLayer_fuse(in_channels, hidden_channels, 1, 1, bias=bias, act=act)
#         self.retention = retention
#         self.bottlenecks = RetBlock(retention='chunk', embed_dim=hidden_channels, num_heads=num_heads, ffn_dim=hidden_channels)
#         self.RelPos = RelPos2d(embed_dim=hidden_channels, num_heads=num_heads, initial_value=2, heads_range=4)
#         if hidden_channels != out_channels:
#             self.conv3 = ConvNormLayer_fuse(hidden_channels, out_channels, 1, 1, bias=bias, act=act)
#         else:
#             self.conv3 = nn.Identity()        
...
#         x_2 = self.conv2(x)
#         x_1 = self.conv1(x)
#         x_1 = self.bottlenecks(x_1.permute(0, 2, 3, 1), None, self.retention=='chunk', rel_pos).permute(0, 3, 1, 2)
#         return self.conv3(x_1 + x_2)


# class Retrepncspelan4(nn.Module):
#     def __init__(self, c1, c2, c3, c4, n=3,
#                  bias=False,
#                  act="silu"):
#         super().__init__()
#         self.c = c3//2
#         self.cv1 = ConvNormLayer_fuse(c1, c3, 1, 1, bias=bias, act=act)
#         self.cv2 = nn.Sequential(retCSPLayer(c3//2, c4, n, 1, bias=bias, act=act), ConvNormLayer_fuse(c4, c4, 3, 1, bias=bias, act=act))
#         self.cv3 = nn.Sequential(retCSPLayer(c4, c4, n, 1, bias=bias, act=act), ConvNormLayer_fuse(c4, c4, 3, 1, bias=bias, act=act))
#         self.cv4 = ConvNormLayer_fuse(c3+(2*c4), c2, 1, 1, bias=bias, act=act)
#     def forward_chunk(self, x):
#         y = list(self.cv1(x).chunk(2, 1))
#         y.extend((m(y[-1])) for m in [self.cv2, self.cv3])
...
@register()
class HybridEncodermobilemamba(nn.Module):
    ...
    def __init__(self,
        ...
        global_rat=[0.8, 0.7, 0.6]
        local_rat = [0.2, 0.2, 0.3]
